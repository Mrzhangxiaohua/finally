...
class NN:
    ...
    def create(self):
        ...
        """
        # input
        img_repr_s = Input(shape=(2048,), name='souce_input')
        img_repr_t = Input(shape=(2048,), name='target_input')
        # layers
    ...
    def create_img_repr(self, weights_file, gen, save_name, max_n_imgs):
        ...
        """
        res = self.Resnet(weights_file)
        namei = res.layers[175].name
        resnet_repr = Model(input=res.input,
                           output=res.get_layer(namei).output)
        resnet_repr.compile(loss='categorical_crossentropy',
                           optimizer='sgd',
                           metrics=['categorical_accuracy'])
        ...
        if not isfile(self.exp_folder+save_name+'_img_repr.npy'):
            ...
            while True:
                ...
                img_repr = resnet_repr.predict(x)
                if is_first:
...
#         print("========================================")
    ...
    def Resnet(self, weights_path=None):
