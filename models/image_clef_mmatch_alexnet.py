...
from keras.layers import Dense, Input, Dropout, Flatten, Activation, merge, Lambda
...
class NN:
    ...
    def create_img_repr(self, weights_file, gen, save_name, max_n_imgs):
        ...
        if not isfile(self.exp_folder + save_name + '_img_repr.npy'):
            ...
            np.save(self.exp_folder + save_name + '_img_repr.npy', repres)
            np.save(self.exp_folder + save_name + '_labels.npy', labels)
        else:
            print('Loading image representations of ' + str(save_name) + '..')
            repres = np.load(self.exp_folder + save_name + '_img_repr.npy')
    ...
    def AlexNet(self, weights_path=None):
        ...
        conv_1 = Convolution2D(nb_filter=96, nb_row=11, nb_col=11, activation='relu', subsample=(4, 4), name='conv_1')(
            inputs)
